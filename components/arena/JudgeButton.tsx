...
export function JudgeButton({ onClick, loading }: JudgeButtonProps) {
  return (
    <div className="flex justify-center my-[var(--space-6)]">
      <button
        onClick={onClick}
        disabled={loading}
        className={`h-10 px-[var(--space-6)] rounded-[var(--radius-button)] text-[13px] font-medium tracking-[0.01em] transition-colors cursor-pointer focus-ring flex items-center justify-center gap-2 select-none
          ${loading
